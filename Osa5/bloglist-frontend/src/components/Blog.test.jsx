...
test('lessInfoDiv is visible att start', () => {
  const { container } = render(<Blog blog={blog} username={username} isCurrentUser={false} />)
  const div = container.querySelector('#lessInfoDiv')
  // screen.debug(div)
...
test('clicking the view button hides lessInfoDiv and shows moreInfoDiv', async () => {
  ...
  const { container } = render(<Blog blog={blog} username={username} isCurrentUser={false} toggleVisibility={mockHandler}/>)
...
test('clicking the like button twice calls event handler twice', async () => {
  const mockHandler = vi.fn()
  const { container } = render(<Blog blog={blog} username={username} isCurrentUser={false} updateLike={mockHandler}/>)
  const user = userEvent.setup()
